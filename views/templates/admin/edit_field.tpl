						...
						<div class="panel" id="fieldset_0">
							...
							<div class="form-wrapper">
								<div class="form-group hide">
									<input type="hidden" name="id_field" id="id_field"
										value="{$fields_value['id_field']|escape:'htmlall':'UTF-8'}">
								</div>
								<div class="form-group hide">
									<input type="hidden" name="sort_order" id="sort_order"
										value="{$fields_value['sort_order']|escape:'htmlall':'UTF-8'}">
								</div>
								<div class="form-group">
									...
									<div class="col-lg-5">
										<input type="text" name="field_name" id="field_name"
											value="{$fields_value['field_name']|escape:'htmlall':'UTF-8'}" class=""
								...
								<div class="form-group">
									...
									<div class="col-lg-2">
										<select name="field_type" class=" fixed-width-xl" id="field_type"
											...
											<option value="text" {if isset($fields_value['field_type']) AND
												$fields_value['field_type']} {if $fields_value['field_type'] eq 'text' }
												selected="selected" {/if} {/if}>{l s='text'
												mod='addifyb2bregistrationformbuilder'}</option>

											<option value="textarea" {if isset($fields_value['field_type']) AND
												$fields_value['field_type']} {if $fields_value['field_type']
												eq 'textarea' } selected="selected" {/if} {/if}>{l s='textarea'
												mod='addifyb2bregistrationformbuilder'}</option>

											<option value="number" {if isset($fields_value['field_type']) AND
												$fields_value['field_type']} {if $fields_value['field_type'] eq 'number'
												} selected="selected" {/if} {/if}>{l s='number'
												mod='addifyb2bregistrationformbuilder'}</option>

											<option value="email" {if isset($fields_value['field_type']) AND
												$fields_value['field_type']} {if $fields_value['field_type'] eq 'email'
												} selected="selected" {/if} {/if}>{l s='email'
												mod='addifyb2bregistrationformbuilder'}</option>

											<option value="password" {if isset($fields_value['field_type']) AND
												$fields_value['field_type']} {if $fields_value['field_type']
												eq 'password' } selected="selected" {/if} {/if}>{l s='password'
												mod='addifyb2bregistrationformbuilder'}</option>

											<option value="radiobutton" {if isset($fields_value['field_type']) AND
												$fields_value['field_type']} {if $fields_value['field_type']
												eq 'radiobutton' } selected="selected" {/if} {/if}>{l s='radiobutton'
												mod='addifyb2bregistrationformbuilder'}</option>

											<option value="select" {if isset($fields_value['field_type']) AND
												$fields_value['field_type']} {if $fields_value['field_type'] eq 'select'
												} selected="selected" {/if} {/if}>{l s='select'
												mod='addifyb2bregistrationformbuilder'}</option>

											<option value="fileupload" {if isset($fields_value['field_type']) AND
												$fields_value['field_type']} {if $fields_value['field_type']
												eq 'fileupload' } selected="selected" {/if} {/if}>{l s='fileupload'
												mod='addifyb2bregistrationformbuilder'}</option>
											<option value="switch" {if isset($fields_value['field_type']) AND
												$fields_value['field_type']} {if $fields_value['field_type']
												eq 'textarea' } selected="selected" {/if} {/if}>{l s='switch'
												mod='addifyb2bregistrationformbuilder'}</option>

											<option value="multiselect" {if isset($fields_value['field_type']) AND
												$fields_value['field_type']} {if $fields_value['field_type']
												eq 'multiselect' } selected="selected" {/if} {/if}>{l s='multiselect'
												mod='addifyb2bregistrationformbuilder'}</option>

											<option value="checkbox" {if isset($fields_value['field_type']) AND
												$fields_value['field_type']} {if $fields_value['field_type']
												eq 'checkbox' } selected="selected" {/if} {/if}>{l s='checkbox'
												mod='addifyb2bregistrationformbuilder'}</option>

											<option value="multicheckbox" {if isset($fields_value['field_type']) AND
												$fields_value['field_type']} {if $fields_value['field_type']
												eq 'multicheckbox' } selected="selected" {/if} {/if}>{l
												s='multicheckbox' mod='addifyb2bregistrationformbuilder'}</option>

											<option value="color" {if isset($fields_value['field_type']) AND
												$fields_value['field_type']} {if $fields_value['field_type'] eq 'color'
												} selected="selected" {/if} {/if}>{l s='color'
												mod='addifyb2bregistrationformbuilder'}</option>

											<option value="date" {if isset($fields_value['field_type']) AND
												$fields_value['field_type']} {if $fields_value['field_type'] eq 'date' }
												selected="selected" {/if} {/if}>{l s='date'
												mod='addifyb2bregistrationformbuilder'}</option>

											<option value="time" {if isset($fields_value['field_type']) AND
												$fields_value['field_type']} {if $fields_value['field_type'] eq 'time' }
								...
								<div class="form-group">
									...
									<div class="col-lg-5">
										<input type="text" name="placeholder" id="placeholder"
											value="{$fields_value['placeholder']|escape:'htmlall':'UTF-8'}" class=""
								...
								<div class="form-group">
									...
									<div class="col-lg-5">
										<input type="text" name="field_options" id="field_options"
											value="{$fields_value['field_options']|escape:'htmlall':'UTF-8'}" class=""
								...
								<div class="form-group">
									...
									<div class="col-lg-5">
										<textarea type="textarea" label="Description" name="description"
											required="false" cols="60" rows="10">{$fields_value['description']|escape:'htmlall':'UTF-8'}</textarea>
									</div>
								</div>
							...
							<div class="panel-footer">
								<button type="submit" value="1" id="module_form_submit_btn"
									name="submitAddifyb2bregistrationformbuilderfields"
